...
// Função para gerar os Harmônicos Espéricos Normalizados
double fWnorm(double th, double ph, WKQ* Wkq, double delta)
{
  double PI = 3.1415926;
  if(th>180.){th=360.-th; ph=ph+180.;}
  double phi=PI*ph/180.; // phi em radianos
  double theta = PI*th/180.;
  double x=cos(PI*th/180.);
  complex<double> Wc(0.,0.);
//  double sqrtInv4pi = 0.28209479177;
  double dt = delta*PI/180.;
  ...
  for(int k=0;k<=4;k+=2)
  {
    for(int q=-k;q<=k;q++)
    {
      ...
      complex <double> cf(re,im);
      double CNorm = 0.28209479177*sqrt((2*k+1)*fact(k-m)/fact(k+m));
      double fSolAng = dt*dt*(theta+dt/2.0)/sin(theta+dt/2.0);
      Wc += Wkq->get(k,q)*CNorm*fSolAng*gsl_sf_legendre_sphPlm(k,abs(m),x)*cf;
    }
  }
  return Wc.real();   
...
//    cout << " Wc Norm: " << fWnorm(theta,phi,&Wkq,delta) << endl;
  }
  ...
  double delta = 10.0;
  for(phi = 0.0; phi <= 360.0; phi = phi+delta)
  {
    for(theta = 0.0; theta <= 180.0; theta = theta+delta)
      saida << setw(10) << fW(theta,phi,&Wkq) << " "; 
//      saida << setw(10) << fWnorm(theta,phi,&Wkq,delta) << " "; 
    saida << endl;
  }   
  saida.close();
